(() => {
  ...
  function renderHip() {
    ...
    if (role === 'ems')
      hip.innerHTML =
        record() +
        `<button class="primary" data-action="send" ${sent ? 'disabled' : ''}>${sent ? 'Handoff sent ✓' : 'Send to hospital <span class="control-arrow" aria-hidden="true">→</span>'}</button>`;
    if (role === 'charge')
      hip.innerHTML = sent
        ? record() +
          `<button class="primary" data-action="assign" ${assigned ? 'disabled' : ''}>${assigned ? 'Resources assigned ✓' : 'Assign bed, nurse & monitor'}</button><div class="demo-record"><strong>Room 06 / Turnover</strong><p>${cleaned ? 'Alex assigned · cleaning queued' : 'Cleaning team assignment needed'}</p><button class="demo-link" data-action="clean" ${cleaned ? 'disabled' : ''}>${cleaned ? 'Cleaner assigned ✓' : 'Assign Alex to room 06 <span class="control-arrow" aria-hidden="true">→</span>'}</button></div>`
  ...
  function renderPlexi() {
    if (!side) {
      plexi.innerHTML =
        '<h4 class="demo-question">What brings you here?</h4><p>Find someone for the job, or put your skills to work.</p><div class="role-choices"><button data-role="requester"><strong>I need a hand <span class="control-arrow" aria-hidden="true">↗</span></strong><span>Post a task and find help.</span></button><button data-role="helper"><strong>I can help <span class="control-arrow" aria-hidden="true">↗</span></strong><span>Browse tasks and offer your skills.</span></button></div>';
      return;
    }
    plexi.innerHTML = `<div class="dashboard-heading"><h4>${side === 'helper' ? 'Find your next task' : 'Your task board'}</h4><button class="demo-link" data-switch>Switch role <span class="control-arrow" aria-hidden="true">↗</span></button></div>`;
    if (side === 'helper')
      plexi.innerHTML += `<div class="demo-record"><span class="meta">Home / Topeka</span><h4>Assemble a bookshelf</h4><p>Saturday afternoon · $45 budget</p><button class="primary" data-offer ${offered ? 'disabled' : ''}>${offered ? 'Offer sent ✓' : 'Offer to help <span class="control-arrow" aria-hidden="true">→</span>'}</button></div><div class="demo-record"><strong>Your offers / ${offered ? '1 pending' : 'None yet'}</strong><p>${offered ? 'Bookshelf assembly · awaiting a reply' : 'Choose a task to get started.'}</p></div>`;
    else {
      plexi.innerHTML +=
        '<form id="plexi-task-form" class="task-form"><label>What do you need help with?<input name="task" required maxlength="80" placeholder="e.g. Set up my new printer"></label><button class="primary" type="submit">Add sample task <span class="control-arrow" aria-hidden="true">→</span></button></form><div id="plexi-tasks"></div>';
